...
public sealed class HelcimCheckoutContextRepository : DbRepository, IHelcimCheckoutContextRepository
{
    ...
    public async Task Save(HelcimCheckoutContext context)
    {
        ...
        command.CommandText = @"INSERT INTO helcim_checkout_context (InvoiceNumber, UserId, FamilyId, SaveCardInfo, ExpiresAt, IsActive)
            VALUES (@Invoice, @User, @Family, @Save, DATE_ADD(UTC_TIMESTAMP(), INTERVAL 1 HOUR), 1)
            ON DUPLICATE KEY UPDATE UserId=@User, FamilyId=@Family, SaveCardInfo=@Save, ExpiresAt=VALUES(ExpiresAt), IsActive=1";
    ...
    {
        using var connection = await Database.CreateAndOpenConnectionAsync(); using var command = connection.CreateCommand();
        command.CommandText = "SELECT InvoiceNumber, UserId, FamilyId, SaveCardInfo FROM helcim_checkout_context WHERE InvoiceNumber=@Invoice AND IsActive=1 AND ExpiresAt >= UTC_TIMESTAMP()";
