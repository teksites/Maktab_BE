...
public class CourseGroupPreRequisiteServiceTests
{
    ...
    [Fact]
    public async Task GetByEnrollment_DelegatesToRepository()
    {
        var enrollmentId = Guid.NewGuid();
        var expected = new[]
        {
            CreateResponse(Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid())
        };

        var repository = new Mock<ICourseGroupPreRequisiteRepository>();
        repository
            .Setup(repo => repo.GetByEnrollment(enrollmentId, false))
            .ReturnsAsync(expected);

        var service = new CourseGroupPreRequisiteService(repository.Object);

        var result = await service.GetByEnrollment(enrollmentId, false);

        Assert.Equal(expected, result);
    }

    [Fact]
    public async Task Update_DelegatesToRepository()
    {
        ...
        var repository = new Mock<ICourseGroupPreRequisiteRepository>();
        repository
            .Setup(repo => repo.Update(prerequisiteId, request))
            .ReturnsAsync(true);

        var service = new CourseGroupPreRequisiteService(repository.Object);

        var result = await service.Update(prerequisiteId, request);
